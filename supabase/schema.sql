...
create table if not exists public.users (
  ...
  daily_max_booking_hours numeric(4,2) not null default 3 check (daily_max_booking_hours > 0 and daily_max_booking_hours <= 24),
  created_at timestamptz not null default now()
);

alter table public.users
  add column if not exists daily_max_booking_hours numeric(4,2) not null default 3
  check (daily_max_booking_hours > 0 and daily_max_booking_hours <= 24);
...
declare
  ...
  booking_user public.users%rowtype;
  local_start timestamp;
  local_end timestamp;
  requested_hours numeric;
  daily_booked_hours numeric;
  user_daily_booked_hours numeric;
...
begin
  ...
  select * into booking_user from public.users where id = new.user_id for share;
  if not found then raise exception using errcode = 'P0001', message = '존재하지 않는 사용자입니다.'; end if;
  ...
  if daily_booked_hours + requested_hours > target_room.max_booking_hours then
    raise exception using errcode = 'P0001', message = '특별실 하루 최대 예약 시간을 초과했습니다.';
  end if;
  if user_daily_booked_hours + requested_hours > booking_user.daily_max_booking_hours then
    raise exception using errcode = 'P0001', message = '개인 하루 최대 예약 시간을 초과했습니다.';
  end if;
...
-- 교사/관리자 전용 실제 DB 통계
drop function if exists public.get_room_daily_booking_usage(bigint, date, bigint);
...
declare
  ...
  user_booked_hours numeric;
begin
  if auth.uid() is null then
    raise exception using errcode = '42501', message = '로그인이 필요합니다.';
  end if;
  if target_user_id is not null and target_user_id <> auth.uid() and not public.is_staff() then
    raise exception using errcode = '42501', message = '다른 사용자의 예약 한도를 조회할 권한이 없습니다.';
  end if;
  ...
  select * into target_user from public.users where id = effective_user_id;
  if not found then
    raise exception using errcode = 'P0001', message = '존재하지 않는 사용자입니다.';
  end if;
  ...
  where b.room_id = target_room_id
    ...
    and b.start_time < day_end;
  select coalesce(sum(extract(epoch from (b.end_time - b.start_time)) / 3600.0), 0)
    into user_booked_hours
  from public.bookings b
  where b.user_id = effective_user_id
    and public.booking_blocks_daily_limit(b.status)
    and (exclude_booking_id is null or b.id <> exclude_booking_id)
    and b.start_time >= day_start
    and b.start_time < day_end;
  ...
  return jsonb_build_object(
    ...
    'user_booked_hours', user_booked_hours,
    'user_daily_max_booking_hours', target_user.daily_max_booking_hours,
    'user_remaining_hours', greatest(target_user.daily_max_booking_hours - user_booked_hours, 0)
  );
end;
$$;
...
revoke all on function public.get_room_daily_booking_usage(bigint, date, bigint, uuid) from public;
...
grant execute on function public.get_room_daily_booking_usage(bigint, date, bigint, uuid) to authenticated;
